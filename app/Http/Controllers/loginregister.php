...
class loginregister extends Controller
{
    ...
    public function index()
    {
        ...
        $policePositions = PolicePosition::whereIn('position_id', [2, 11])->get();

        $states = states::all();
        $districss = districss::all();
        $countries = countries::all();
        $city = city::all();
        return view('frontend.Login', compact('data','policestation','states','districss','countries','city','policePositions'));
    ...
    public function store(Request $request)
    {
        ...
        $record->police_dob = "1990-01-01";
        $record->police_adhar = "0000 0000 0000";
        $record->police_mobile1 = "0000000000";
        $record->police_mobile2 = '0000000000';
        $record->police_photo_path = " ";
        $record->police_address = " ";
        $record->police_latitude = '0';
        $record->police_longitude = '0';

        $record->country_id  = "1";
        $record->state_id = "1";
        $record->district_id = "1";
        $record->city_id = "1";  
        
        if ($request['position_id'] == 2 || $request['position_id'] == 11) {
            $record->position_id = $request['position_id'];
        } else {
            // Set a session variable to indicate the error
            session()->flash('position_error', 'Please choose the proper position.');
